...
axios.interceptors.request.use(request => {
    request.headers["demo-auth"] = store.state.loginInfo ? store.state.loginInfo.userId : ""; //已将userId保存到store中
    return request
})
// 切换页面取消请求
axios.interceptors.request.use(request => {
...
router.then(lib => {
    lib.defaults.beforeEach((to, from, next) => {
        source.cancel()
...
axios.interceptors.response.use(response => {
    ...
    if ([10002].includes(data.ret)) {
        router.then(lib => lib.defaults.push({name: Login}))  //跳转到登录页面
